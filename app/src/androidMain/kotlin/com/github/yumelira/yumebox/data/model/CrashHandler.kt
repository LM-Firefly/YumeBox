...
class CrashHandler private constructor(private val context: Context) : Thread.UncaughtExceptionHandler {
    ...
    private fun saveCrashInfo2File(ex: Throwable) {
        ...
        sb.append(result)
        // Append recent in-memory logs to aid debugging
        try {
            val recentLogs = com.github.yumelira.yumebox.data.model.AppLogBuffer.getSnapshot()
            if (recentLogs.isNotEmpty()) {
                sb.append("\n--- Recent App Logs ---\n")
                sb.append(recentLogs.joinToString(separator = "\n"))
                sb.append("\n--- End Recent App Logs ---\n")
            }
        } catch (e: Exception) {
