...
class TrafficStatisticsCollector(
    ...
    private suspend fun getInternalTrafficTotal(): Pair<Long, Long> {
        return try {
            ...
            val controller = session.externalController?.takeIf { it.isNotEmpty() }
                ?: ClashCore.queryOverride(Clash.OverrideSlot.Persist).externalController?.takeIf { it.isNotEmpty() }
            if (!controller.isNullOrEmpty()) {
                val secret = session.secret.orEmpty().ifEmpty { ClashCore.queryOverride(Clash.OverrideSlot.Persist).secret.orEmpty() }
                val request = Request.Builder()
                    .url("http://$controller/connections")
                ...
                response.use {
                    if (it.isSuccessful) {
                        ...
                        val data = json.decodeFromString<ConnectionsResponse>(bodyString)
                        return data.uploadTotal to data.downloadTotal
                    }
                }
            }
            ...
            Timber.tag(TAG).d(e, "Failed to query traffic from API, falling back to JNI")
            val total = TrafficData.from(ClashCore.queryTrafficTotal())
            total.upload to total.download
        }
    }
