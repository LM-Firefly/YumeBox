...
class ProxyFacade(
    ...
    suspend fun loadProfile(profile: Profile): Result<Unit> {
        return clashManager.loadProfile(profile)
    }
    suspend fun reloadCurrentProfile(): Result<Unit> {
        return clashManager.reloadCurrentProfile()
    }
    suspend fun selectProxy(groupName: String, proxyName: String): Result<Boolean> {
        return clashManager.proxyStateRepository.selectProxy(groupName, proxyName, currentProfile.value)
    }
