...
suspend fun downloadAndImportProfileConfig(
    profile: Profile,
    workDir: File,
    force: Boolean = true,
    ...
    return runCatching {
        ...
        val result = importService.importProfile(
            ...
            force = force,
            onProgress = { state ->
                val (message, progress) = mapStateToProgress(profile.type, state)
...
private fun mapStateToProgress(profileType: ProfileType, state: ImportState): Pair<String, Int> {
    return when (state) {
        ...
        is ImportState.Success -> {
            val completeMessage = when (profileType) {
                ProfileType.URL -> MLang.ProfilesVM.Progress.DownloadComplete
                ProfileType.FILE -> MLang.ProfilesVM.Progress.ImportComplete
            }
            completeMessage to 100
        }
