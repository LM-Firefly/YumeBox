...
class YumeFilesProvider : DocumentsProvider() {
    companion object {
    ...
    override fun queryRoots(projection: Array<String>?): Cursor {
        return MatrixCursor(projection ?: DEFAULT_ROOT_COLUMNS).apply {
            newRow().apply {
                ...
                add(DocumentsContract.Root.COLUMN_ICON, R.drawable.ic_logo_service)
                add(DocumentsContract.Root.COLUMN_TITLE, context?.getString(R.string.yumebox_documents_title) ?: "YumeBox")
                add(DocumentsContract.Root.COLUMN_SUMMARY, context?.getString(R.string.yumebox_documents_summary) ?: "Data")
    ...
    override fun queryChildDocuments(
        ...
        val parentFile = if (parentDocumentId == "/") {
            // expose app files dir — you can change to a subfolder like File(context.filesDir, "data")
            context?.filesDir
        } else {
