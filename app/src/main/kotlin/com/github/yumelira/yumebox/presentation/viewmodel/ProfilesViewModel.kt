...
class ProfilesViewModel(
    ...
    suspend fun downloadProfileAndSave(profile: Profile, saveToDb: Boolean = true): Profile? {
        ...
        return try {
            ...
            val result = profilesRepository.downloadAndMergeSubscriptionInfo(
                profile = profile,
            ...
            if (result.isSuccess) {
                ...
                if (saveToDb) {
                    if (existingProfile != null) {
                        profilesRepository.updateProfile(finalProfile)
                    } else {
                        profilesRepository.addProfile(finalProfile)
                    }
                }
                finalProfile
            } else {
                ...
                } else if (error?.message?.contains("already downloading") == true) {
                    MLang.ProfilesVM.Progress.DownloadFailed.format("下载已在进行中")
                } else {
    ...
    fun downloadProfileInViewModel(profile: Profile, saveToDb: Boolean = true) = viewModelScope.launch {
        try {
            downloadProfileAndSave(profile, saveToDb)
        } catch (e: kotlinx.coroutines.CancellationException) {
