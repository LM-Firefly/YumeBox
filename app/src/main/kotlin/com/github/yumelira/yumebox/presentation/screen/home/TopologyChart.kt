...
private fun calculateSankeyLayout(
    ...
    // 2. Reuse pre-measured text heights for all nodes
    ...
    // Calculate scale factor considering both value and text height
    val maxLayerTotalValue = layers.values.maxOfOrNull { layerNodes -> 
        layerNodes.fold(0f) { acc, node ->
            acc + max(node.value, textHeightByNodeId[node.id] ?: 0f)
        }
    } ?: 0f
    ...
    val sortedLinks = sankeyData.links.sortedWith(compareBy<Link> {
        nodeById[it.source]?.layer ?: 0
    }.thenBy {
        nodeById[it.source]?.y ?: 0f
    }.thenBy {
        nodeById[it.target]?.y ?: 0f
    })
    val renderLinks = sortedLinks.mapNotNull { link ->
        val source = nodeById[link.source]
        val target = nodeById[link.target]
        if (source != null && target != null) {
            // Calculate link height proportionally based on source and target node actual heights
            ...
            val path = Path().apply {
                ...
                cubicTo(
                    startX + (endX - startX) / 2, endY + targetLinkHeight,
                    startX + (endX - startX) / 2, startY + sourceLinkHeight,
                    startX, startY + sourceLinkHeight
                )
                close()
            }
    ...
    // 5. Prepare Drawables (Nodes & Text)
