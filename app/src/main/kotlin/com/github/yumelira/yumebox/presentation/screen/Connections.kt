    ...
    Scaffold(
        ...
        Column(
                    ...
                    Card(
                        ...
                        Column(
                            modifier = Modifier
                                ...
                                .clickable {
                                    navigator.navigate(OverrideScreenDestination(openControllerAddress = true))
                                }
                                .background(MiuixTheme.colorScheme.errorContainer)
                                .padding(16.dp)
                        ) {
                            Text(
                                text = MLang.Connections.Error.Title.format(connectionError),
                                style = MiuixTheme.textStyles.body2,
                                color = MiuixTheme.colorScheme.onErrorContainer
                            )
                            Spacer(modifier = Modifier.height(4.dp))
                            Text(
                                text = MLang.Connections.Error.CheckSettings,
            ...
            HorizontalPager(
                ...
                if (pageConnections.isEmpty() && connectionError == null) {
                    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                        Column(horizontalAlignment = Alignment.CenterHorizontally) {
                            Text(text = "暂无连接或内核未运行", style = MiuixTheme.textStyles.body1)
                            Spacer(modifier = Modifier.height(8.dp))
                            Text(text = "请检查配置或打开连接页面以刷新", style = MiuixTheme.textStyles.body2)
                        }
                    }
                } else {
                    ScreenLazyColumn(
                        ...
                        items(items = pageConnections, key = { it.id }) { connection ->
                            ConnectionItem(
...
fun ConnectionItem(connection: Connection, onClose: () -> Unit, modifier: Modifier = Modifier, onClick: () -> Unit) {
    ...
    // Precompute render data only when connection data changes
    val renderData = remember(connection) {
        runCatching {
