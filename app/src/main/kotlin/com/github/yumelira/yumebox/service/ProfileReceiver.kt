        ...
        when (intent.action) {
            ...
            ACTION_PROFILE_REQUEST_UPDATE -> {
                ...
                if (profileId != null && profileId.isNotBlank()) {
                    Timber.tag(TAG).i("=== Triggered profile auto-update: %s ===", profileId)
    ...
    companion object {
        ...
        fun scheduleRetry(context: Context, profileId: String, minutesFromNow: Long) {
            try {
                ...
                Timber.tag(TAG).w("Scheduled RETRY for profile %s in %d minutes (failed update will retry)", profileId, minutesFromNow)
            } catch (e: Exception) {
