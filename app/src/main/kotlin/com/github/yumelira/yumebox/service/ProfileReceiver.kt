    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.action) {
            ...
            Intent.ACTION_TIMEZONE_CHANGED, Intent.ACTION_TIME_CHANGED -> {
                // Start a foreground service to reschedule updates (best-effort)
                try {
                    val serviceIntent = Intent(context, ProfileAutoUpdateService::class.java)
                    context.startForegroundService(serviceIntent)
                } catch (t: Throwable) {
                    // If we cannot start service (pre-boot), just attempt reschedule in background
                    Timber.tag(TAG).w(t, "Failed to start ProfileAutoUpdateService for reschedule; trying direct reschedule")
                    GlobalContext.getOrNull()?.get<ProfilesStore>()?.let { store ->
