...
val packagingAbiList = if (isMergeBuild) extensionAbiList else appAbiList
val localeList = gropify.locale.app.list.split(",").map { it.trim() }

android {
    namespace = appNamespace
    compileSdk = gropify.android.compileSdk
    ndkVersion = gropify.android.ndkVersion
    ...
    sourceSets {
        named("main") {
            ...
            kotlin.directories.add("build/generated/fytxt/kotlin/commonMain/kotlin")
        }
    }
...
dependencies {
    ...
    implementation("com.caoccao.javet:javet-node-android:5.0.5")
